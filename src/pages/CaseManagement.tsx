...
> = {
  ...
  in_progress: {
    label: "In Progress",
    color: "#2D7FD6",
    bg: "rgba(45,127,214,0.12)",
    border: "rgba(45,127,214,0.3)",
    icon: RotateCw,
  },
  closed: {
    label: "Closed",
...
function enrichCases(cases: CaseOut[], screeningMap: Map<string, ScreeningListItem>): EnrichedCase[] {
  return cases.map((c, idx) => {
    ...
    const caseRef = `${String(c.id).slice(0, 8).toUpperCase()}-${(c.case_type || "KYC")
      .slice(0, 3)
      .toUpperCase()}`;
  ...
  async function pick(next: CaseStatus) {
    ...
    try {
      const apiStatus: CaseWorkflowStatus =
        next === "pending" ? "PENDING" : next === "in_progress" ? "IN_PROGRESS" : "CLOSED";
            ...
            {(["pending", "in_progress", "closed"] as CaseStatus[]).map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{c.label}</span>
                  {s === current && <Check size={13} strokeWidth={2.8} style={{ marginLeft: "auto" }} />}
                </button>
              );
            })}
          </div>
...
export default function CaseManagement() {
  ...
  function load() {
    setBusy(true);
    Promise.allSettled([listCases({}), listScreenings({ limit: 200, offset: 0 })])
      .then(([casesRes, screeningsRes]) => {
        ...
        const screenings: ScreeningListItem[] =
          screeningsRes.status === "fulfilled" ? screeningsRes.value.items : [];
  ...
  async function bulkUpdateStatus(status: CaseWorkflowStatus) {
    ...
    setCases((prev) =>
      prev.map((c) => (selectedIds.has(c.id) ? { ...c, status: mapped as CaseStatus } : c))
    );
      ...
      >
        <div>
          ...
          <div className="page-subtitle">
            {busy ? "Chargement…" : `${cases.length} case${cases.length !== 1 ? "s" : ""} au total`}
          </div>
        </div>
      ...
      <div className="stat-pills">
        ...
        <div
          ...
          style={{
            ...
            borderColor: STATUS_CONFIG.pending.border,
        ...
        <div
          className="stat-pill"
          style={{
            background: STATUS_CONFIG.in_progress.bg,
            borderColor: STATUS_CONFIG.in_progress.border,
        ...
        <div
          ...
          style={{
            ...
            borderColor: STATUS_CONFIG.closed.border,
        ...
        <div className="cases-table-wrapper">
          <table className="cases-table">
            ...
            <tbody>
              {busy ? (
                <tr>
                  <td colSpan={9} style={{ textAlign: "center", padding: "30px 0", opacity: 0.4 }} className="small">
                    Chargement…
                  </td>
                </tr>
              ) : pageItems.length === 0 ? (
                  ...
                  <td colSpan={9}>
                    <div className="empty-state">
                      <div className="empty-state-icon" style={{ display: "flex", justifyContent: "center" }}>
                        <FolderOpen size={30} strokeWidth={2.1} />
                      </div>
                ...
                pageItems.map((c) => (
                  <tr key={c.id} className={selectedIds.has(c.id) ? "selected" : ""}>
                    ...
                    <td>
                      <StatusChanger caseId={c.id} current={c.status} onChange={handleStatusChange} />
                    </td>

                    <td>
                      <div className="assignee-cell">
                        <div className="entity-avatar" style={{ width: 24, height: 24, fontSize: 9 }}>
                          {c.assigneeInitials}
                        </div>
                    ...
                    <td>
                      <Link to={caseLink(c)} className="btn secondary sm">
                        View
                      </Link>
                    </td>
                  </tr>
