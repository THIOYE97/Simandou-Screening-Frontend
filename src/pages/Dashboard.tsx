...
// ✅ Risk from real risk_level field, fallback to status heuristic
...
// ✅ Breakdown by recommended_action + matches
function buildBreakdown(items: ScreeningListItem[]) {
  const withMatches  = items.filter(i => (i.matches_count ?? 0) > 0).length;
  const high         = items.filter(i => riskOf(i) === "high").length;
  const medium       = items.filter(i => riskOf(i) === "medium").length;
  const pep          = items.filter(i => String(i.recommended_action ?? "").toUpperCase().includes("REVIEW") || riskOf(i) === "high").length;
  const sanctions    = items.filter(i => riskOf(i) === "high").length;
  const adverseMedia = items.filter(i => riskOf(i) === "medium").length;

  return [
    { label: "Sanctions",           icon: "⚖️", color: "#E84040", bg: "rgba(232,64,64,0.12)",    count: sanctions },
    { label: "Watchlists",          icon: "📋", color: "#2D7FD6", bg: "rgba(45,127,214,0.15)",   count: withMatches },
    { label: "Adverse Media",       icon: "📰", color: "#F5920A", bg: "rgba(245,146,10,0.12)",   count: adverseMedia },
    { label: "PEP & Relatives",     icon: "👤", color: "#2ECC8F", bg: "rgba(46,204,143,0.12)",   count: pep },
    { label: "Politically Exposed", icon: "🏛️", color: "#A78BFA", bg: "rgba(167,139,250,0.12)",  count: Math.round(pep * 0.6) },
  ];
}
...
function buildChartData(items: ScreeningListItem[]): ChartPoint[] {
  ...
  const now  = Date.now();
  const pts: ChartPoint[] = Array.from({ length: 5 }, (_, i) => ({ label: `S-${4 - i}`, high: 0, medium: 0, low: 0 }));
...
function LineChart({ data }: { data: ChartPoint[] }) {
  const W = 400; const H = 130; const P = 20;
  ...
  function pts(k: "high" | "medium" | "low") {
    return data.map((p, i) => `${P + (i / n) * cW},${P + cH - (p[k] / max) * cH}`).join(" ");
  }
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ overflow: "visible" }}>
      {[0.25, 0.5, 0.75, 1].map((t, i) => (
        <line key={i} x1={P} y1={P + cH * (1-t)} x2={W-P} y2={P + cH * (1-t)} stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
      ))}
      {(Object.keys(cols) as ("high"|"medium"|"low")[]).map(k => (
        <polyline key={k} points={pts(k)} fill="none" stroke={cols[k]} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity={0.9} />
      ))}
      {(Object.keys(cols) as ("high"|"medium"|"low")[]).map(k =>
        data.map((p, i) => (
          <circle key={`${k}${i}`} cx={P + (i/n)*cW} cy={P + cH - (p[k]/max)*cH} r="3.5" fill={cols[k]} />
        ))
      )}
      {data.map((p, i) => (
        <text key={i} x={P + (i/n)*cW} y={H-4} fontSize="9" fill="rgba(226,237,255,0.35)" textAnchor="middle">{p.label}</text>
      ))}
    </svg>
  );
}
...
// SVG Donut
// ─────────────────────────────────────────────
function DonutChart({ pending, inProgress, closed }: { pending: number; inProgress: number; closed: number }) {
    ...
    <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
      <div className="donut-container" style={{ width: 140, height: 140, flexShrink: 0 }}>
        <svg viewBox="0 0 140 140" width="140" height="140">
          ...
          {arcs.map((arc, i) => (
            <circle key={i} cx="70" cy="70" r={R} fill="none" stroke={arc.color} strokeWidth="16"
              strokeDasharray={`${arc.dash} ${circ - arc.dash}`}
              strokeDashoffset={-arc.off + circ * 0.25} strokeLinecap="round" />
          ))}
        </svg>
  ...
  useEffect(() => {
    listScreenings({ limit: 200, offset: 0 })
      .then(r => {
        ...
        // ✅ Use real risk_level from backend
    ...
    <>
      ...
      <div className="page-header" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
        ...
        <div className="row" style={{ gap: 10, flexShrink: 0 }}>
          ...
          <Link to="/screenings" className="btn sm">Export Report</Link>
        </div>
      </div>
        ...
        <div className="dashboard-col">
          ...
          <div className="chart-card">
            ...
            <div className="chart-legend">
              {[["high","#E84040",high],["medium","#F5920A",medium],["low","#2ECC8F",low]].map(([k,c,v]) => (
            ...
            {loadSc
              ? <div className="small" style={{ opacity: 0.4, textAlign: "center", padding: "30px 0" }}>Chargement…</div>
          ...
          <div className="chart-card">
            <div className="chart-header">
              ...
              <Link to="/screenings" className="small" style={{ color: "var(--text-accent)" }}>Voir tout →</Link>
            </div>
            <table className="recent-alerts-table">
              <thead>
                <tr><th>Name</th><th>Statut</th><th>Risk</th><th>Date</th></tr>
              </thead>
              <tbody>
                {loadSc
                  ? <tr><td colSpan={4} className="small" style={{ opacity: 0.4, padding: "20px 0" }}>Chargement…</td></tr>
                  : recent.length === 0
                    ? <tr><td colSpan={4} className="small" style={{ opacity: 0.4 }}>Aucun screening.</td></tr>
                    : recent.map(item => {
                        ...
                        return (
                          <tr key={item.id}>
                            ...
                            <td>
                              <span className={`badge ${st==="DONE" ? "badge-ok" : ["RUNNING","PENDING"].includes(st) ? "badge-warn" : "badge-bad"}`}>
                                {item.status || "—"}
                              </span>
                            </td>
                            <td>
                            ...
                            <td className="small" style={{ color:"var(--text-muted)" }}>{fmtDate(item.created_at)}</td>
                          </tr>
                        );
                      })
                }
        ...
        <div className="dashboard-col">
          ...
          <div className="chart-card">
            <div className="chart-header">
              ...
              <Link to="/cases" className="small" style={{ color:"var(--text-accent)" }}>Voir tout →</Link>
            </div>
            {loadCa
              ? <div className="small" style={{ opacity:0.4, textAlign:"center", padding:"30px 0" }}>Chargement…</div>
              : <DonutChart {...caseOvw} />
            ...
            <div className="alerts-breakdown-list">
              {breakdown.map((d, i) => (
                <div className="alert-breakdown-item" key={i}>
                  <div className="alert-breakdown-icon" style={{ background: d.bg }}>{d.icon}</div>
        ...
        <div className="dashboard-col">
          ...
          <div className="chart-card">
            ...
            {breakdown.map((d, i) => (
              <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 0", borderBottom:"1px solid var(--border)" }}>
                <span className="small"><span style={{ marginRight:6 }}>{d.icon}</span>{d.label}</span>
          ...
          <div className="chart-card">
            <div className="chart-header" style={{ marginBottom: 10 }}>
              ...
              <Link to="/watchlists" className="small" style={{ color:"var(--text-accent)" }}>View All</Link>
            </div>
            {[
          ...
          <div className="chart-card">
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              <Link to="/analyst"    className="btn"           style={{ justifyContent:"center" }}>🔍 Nouveau Screening</Link>
              <Link to="/cases"      className="btn secondary" style={{ justifyContent:"center" }}>📁 Voir les Cases</Link>
              <Link to="/screenings" className="btn secondary" style={{ justifyContent:"center" }}>📋 Tous les Screenings</Link>
            </div>
          </div>
        </div>
