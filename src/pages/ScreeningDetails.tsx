// src/pages/ScreeningDetails.tsx — Redesigned professional UI with lucide-react
...
function fmtDate(s: unknown): string {
  if (!s) return "—";
  try {
    return new Date(String(s)).toLocaleDateString("fr-FR", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
...
function riskColor(r: unknown) {
  ...
  if (v === "HIGH") {
    return { bg: "#E84040", text: "white", label: "High Risk", icon: ShieldAlert };
  }
  if (v === "MEDIUM") {
    return { bg: "#F5920A", text: "white", label: "Medium Risk", icon: AlertTriangle };
  }
  if (v === "LOW") {
    return { bg: "#2ECC8F", text: "white", label: "Low Risk", icon: ShieldCheck };
  }
  return { bg: "#475569", text: "white", label: String(r || "—"), icon: ShieldQuestion };
}

function actionStyle(a: unknown) {
  const v = String(a || "").toUpperCase();
  if (v === "PASS") return { color: "#2ECC8F", label: "APPROUVER", icon: CheckCircle2 };
  if (v === "MANUAL_REVIEW") return { color: "#F5920A", label: "REVUE MANUELLE", icon: AlertTriangle };
  if (v === "BLOCK") return { color: "#E84040", label: "BLOQUER", icon: XCircle };
  return { color: "#94A3B8", label: String(a || "—"), icon: ShieldQuestion };
}
...
function deriveCategory(matches: AnyObj[], result: AnyObj | null): string {
  ...
  if ((matches || []).some((m) => String(m.match_band || "").toUpperCase() === "STRONG")) {
...
function pickDocuments(data: AnyObj | null) {
  ...
  return rawDocs.map((d) => ({
    ...
    ocr_confidence:
      typeof (d.ocr_confidence ?? d.ocrConfidence) === "number"
        ? Number(d.ocr_confidence ?? d.ocrConfidence)
        : null,
...
function CaseStatusSelect({ value }: { value: string }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        {labels[cur] || cur}
        <ChevronDown size={12} strokeWidth={2.2} style={{ opacity: 0.8 }} />
      </button>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 7,
                    border: "none",
                    background: cur === o ? `${c.bg}22` : "transparent",
                    color: cur === o ? c.bg : "var(--text-secondary)",
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: "pointer",
                    textAlign: "left",
                ...
                >
                  <span
                    style={{ width: 8, height: 8, borderRadius: 4, background: c.bg, flexShrink: 0 }}
                  />
                  {labels[o] || o}
                </button>
              );
...
function MatchCard({ match }: { match: AnyObj; idx: number }) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
          ...
          >
            <RiskIcon size={14} strokeWidth={2.1} />
            {rc.label}
          </span>
          <div className="small" style={{ opacity: 0.5, display: "flex", alignItems: "center", gap: 6 }}>
            <FileText size={12} />
      ...
      {exp && (
        ...
        >
          ...
          {matchBullets.length > 0 && (
            <details>
              <summary className="badge" style={{ cursor: "pointer", fontSize: 11 }}>
                Pourquoi ce match (technique)
              </summary>
              <div style={{ marginTop: 8, display: "grid", gap: 4 }}>
                {matchBullets.slice(0, 6).map((b: any, i: number) => (
                  <div key={i} className="small" style={{ opacity: 0.8 }}>
                    • {String(b)}
                  </div>
                ))}
              </div>
...
export default function ScreeningDetails() {
  ...
  async function doDecision(decision: "PASS" | "BLOCK") {
    ...
    if (c.length < 4) {
      setDecToast("Commentaire obligatoire (min 4 car.).");
      return;
    }
    if (!id) return;
    setDecBusy(true);
  ...
  function addNote() {
    ...
    setNotes((prev) => [
      ...prev,
      { text: noteText.trim(), time: new Date().toLocaleString("fr-FR"), user: "Moi" },
    ]);
    setNoteText("");
  }
  ...
  const pepMatches = sortedMatches.filter(
    (m) => String(m.match_band || "").toUpperCase() === "STRONG"
  ).length;
  ...
  return (
    <>
      ...
      >
        <Link to="/screenings" style={{ color: "var(--text-muted)", textDecoration: "none" }}>
          Screening Results
        </Link>
        <span>›</span>
      ...
      {!data ? (
        <div className="screen">
          <div className="small" style={{ textAlign: "center", padding: "40px 0", opacity: 0.4 }}>
            {busy ? "Chargement…" : "Aucune donnée."}
          </div>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20, alignItems: "start" }}>
          <div>
            <div className="screen" style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 4 }}>
                    ...
                    {payload?.nationality && <Flag size={16} color="var(--text-muted)" />}
                    <Star size={15} color="var(--text-muted)" style={{ opacity: 0.4 }} />
                  </div>
              ...
              {decToast && (
                <div
                  className={`toast ${decToast.includes("Erreur") || decToast.includes("obligatoire") ? "danger" : "ok"}`}
                  style={{ marginTop: 12 }}
                >
                  {decToast}
                </div>
                  ...
                  {sortedMatches.length === 0 ? (
                    <div className="screen">
                      <div className="empty-state" style={{ padding: "24px 0" }}>
                        <div className="empty-state-icon">
                          <CheckCircle2 size={28} color="#2ECC8F" />
                        </div>
                    ...
                    sortedMatches.map((m, i) => <MatchCard key={i} match={m} idx={i} />)
                  )}
                </div>
                      ...
                      {sortedMatches.slice(0, 3).map((m, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <RIcon2 size={13} strokeWidth={2.1} />
                              {rc2.label}
                            </span>

                            {payload?.nationality && <Flag size={16} style={{ flexShrink: 0 }} />}
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="screen">
                  ...
                  >
                    ...
                    <div className="row" style={{ gap: 8 }}>
                      <button
                        className="btn"
                        style={{ background: "#2ECC8F", borderColor: "#2ECC8F", display: "inline-flex", alignItems: "center", gap: 6 }}
                        disabled={decBusy || comment.trim().length < 4}
                        onClick={() => doDecision("PASS")}
                      >
            ...
            {tab === "details" && (
              <div className="screen">
                  ...
                  {[
                    ...
                    <div key={label} style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                      ...
                      <div
                        ...
                        style={{
                          fontFamily: label === "Request ID" || label === "Case ID" ? "monospace" : "inherit",
                          fontSize: label === "Request ID" ? 11 : undefined,
                        }}
                      >
                        {value}
                      </div>
                  ...
                  <div style={{ marginTop: 20 }}>
                    ...
                    {docs.map((d, i) => (
                      ...
                      >
                        ...
                        {d.download_url && (
                          <a className="btn secondary sm" href={d.download_url} target="_blank" rel="noreferrer">
                            <Download size={14} />
                          </a>
                        )}
                        {d.preview_url && (
                          <a className="btn secondary sm" href={d.preview_url} target="_blank" rel="noreferrer">
                            <Eye size={14} />
                          </a>
                        )}
                      </div>
                ...
                <details style={{ marginTop: 16 }}>
                  ...
                  <textarea
                    readOnly
                    style={{ marginTop: 8, minHeight: 200, fontFamily: "monospace", fontSize: 11 }}
            ...
            {tab === "entities" && (
              <div className="screen">
                ...
                {sortedMatches.length === 0 ? (
                  <div className="empty-state">
                    <div className="empty-state-title">Aucune entité liée.</div>
                  </div>
                ) : (
                      ...
                      >
                        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div className="small" style={{ opacity: 0.5 }}>
                              {sb?.label || "Source"} · {sb?.record_type || "—"}
                            </div>
                          </div>
                          <span
                            style={{
                        ...
                        {sb?.links &&
                          Array.isArray(sb.links) &&
                          sb.links.slice(0, 2).map((url: string, j: number) => (
                            <a
                              key={j}
                              href={url}
                              target="_blank"
                  ...
                  {[
                    {
                      ...
                      detail: `Provider: ${request?.provider || "INTERNAL"}`,
                    },
                    {
                      icon: RefreshCw,
                      title: "Analyse en cours",
                      time: fmtDateTime(createdAt),
                      detail: "Moteur de matching actif",
                    },
                    ...(matchesRaw.length > 0
                      ? [
                          {
                            icon: AlertTriangle,
                            title: `${matchesRaw.length} correspondance(s) trouvée(s)`,
                            time: fmtDateTime(request?.completed_at || createdAt),
                            detail: `Risk: ${risk || "—"}`,
                          },
                        ]
                      : []),
                      ...
                      ? [
                          {
                            ...
                            time: fmtDateTime(request.completed_at),
                            detail: `Statut: ${request.status || "DONE"}`,
                          },
                        ]
                      : []),
                    ...decisionHistory.map((d) => ({
                      icon: d.decision === "PASS" ? CheckCircle2 : XCircle,
                      title: `Décision: ${d.decision}`,
                      time: fmtDateTime(d.decided_at),
                    ...
                    return (
                      <div key={i} style={{ display: "flex", gap: 16, marginBottom: 16, position: "relative" }}>
                        ...
                        >
                          <EvIcon size={10} strokeWidth={2.5} />
                        </div>
                        <div>
                          ...
                          {ev.detail && (
                            <div className="small" style={{ marginTop: 3, opacity: 0.8 }}>
                              {ev.detail}
                            </div>
                          )}
                        </div>
          ...
          >
            <div className="chart-card">
              ...
              {caseId && (
                <div className="small" style={{ marginBottom: 4 }}>
                  Case #{" "}
                  <b style={{ fontFamily: "monospace", fontSize: 10 }}>
              ...
              {confidence != null && (
                <div style={{ marginTop: 10 }}>
                  <div className="small" style={{ marginBottom: 4, opacity: 0.6 }}>
            ...
            <div className="chart-card">
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                ...
                />
                <button className="btn sm" onClick={addNote} disabled={!noteText.trim()} style={{ padding: "6px 12px" }}>
                  +
                </button>
              </div>

              {notes.length === 0 ? (
                <div className="small" style={{ opacity: 0.4 }}>
