// src/pages/ScreeningDetails.jsx
...
function statusBadgeClass(status) {
  ...
  if (["RUNNING", "PENDING", "PENDING_REVIEW", "MANUAL_REVIEW"].includes(v))
    return "badge badge-warn";
  if (["FAILED", "REJECTED", "ERROR", "BLOCK"].includes(v)) return "badge badge-bad";
  return "badge";
}

function humanDecision(d) {
...
// ------------------------------------------------------------
function pickBestDocExtractedFieldsFromPayload(payload) {
...
function getDisplayNameFromIdentity(i, payload) {
...
function renderBulletsOrFallback(bullets, fallback) {
...
function renderRawDetails(raw, title = "Voir détails bruts") {
...
function pickDecisionLatest(data) {
  // priorités : snake_case, puis camelCase, puis (fallback legacy si tu l’as gardé)
  return data?.decision_latest ?? data?.decisionLatest ?? data?.analyst_decision_latest ?? null;
}

function pickDecisionHistory(data) {
  const h =
    data?.decision_history ??
    data?.decisionHistory ??
    data?.analyst_decision_history ??
    data?.analystDecisionHistory ??
    [];
...
export default function ScreeningDetails() {
  ...
  const [form, setForm] = useState({ lastName: "", firstName: "", dob: "", docNo: "" });
        ...
        {!data ? (
          ...
          <div className="grid-2">
                ...
                {docs.length > 0 ? (
                  <div style={{ display: "grid", gap: 12 }}>
                    {docs.map((d, idx) => {
                      ...
                      const isPdf =
                        mime === "application/pdf" || (d.original_filename || "").toLowerCase().endsWith(".pdf");
                ...
                {matches.length === 0 ? (
                  ...
                  <div className="match-list">
                    {matches.map((m, idx) => {
                      ...
                      const tone =
                        scoreN != null && scoreN >= 90
                          ? "badge badge-bad"
                          : scoreN != null && scoreN >= 75
                          ? "badge badge-warn"
                          : "badge";
                        ...
                        <div className="match-card" key={idx}>
                          ...
                          <div className="match-section">
                            ...
                            {renderBulletsOrFallback(
                              m.sanctionBullets,
                              sb?.summary || "Aucun motif détaillé dans la source (summary/raw_payload)."
                            )}
                          ...
                          <details className="match-details">
                            ...
                            {renderBulletsOrFallback(
                              m.matchBullets,
                              m.reasonsHuman || "Correspondance détectée par le moteur (détails techniques disponibles)."
                            )}
            ...
            <div className="screen">
              ...
              {showTech ? (
                <div className="card" style={{ marginTop: 12 }}>
                  <div className="h2" style={{ marginTop: 0 }}>Détails techniques (optionnel)</div>
                  <div className="small" style={{ opacity: 0.9, marginBottom: 8 }}>
                    À utiliser uniquement si tu fais du debug.
                  </div>

                  <details>
                    <summary className="badge" style={{ cursor: "pointer" }}>Voir l’objet request (raw)</summary>
                    <div style={{ height: 10 }} />
                  ...
                  <details>
                    <summary className="badge" style={{ cursor: "pointer" }}>Voir l’objet result (raw)</summary>
                    <div style={{ height: 10 }} />
                  ...
                  <details>
                    <summary className="badge" style={{ cursor: "pointer" }}>Voir matches (raw)</summary>
                    <div style={{ height: 10 }} />
                  ...
                  <details>
                    <summary className="badge" style={{ cursor: "pointer" }}>Voir data (raw)</summary>
                    <div style={{ height: 10 }} />
