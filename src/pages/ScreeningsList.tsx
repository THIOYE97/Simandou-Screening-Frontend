...
// Category derived from risk + matches
...
function frStatus(raw?: string|null) {
  ...
  const map: Record<string,string> = { DONE:"Terminé", APPROVED:"Approuvé", RUNNING:"En cours", PENDING:"Pending", FAILED:"Échec", REJECTED:"Rejeté" };
  return map[v]||v||"—";
}
...
function AlertsOverview({ items }: { items: ScreeningListItem[] }) {
  ...
  const sanctions = items.filter(i=>String(i.recommended_action||"").toUpperCase().includes("BLOCK")).length;
  ...
  return (
    <div className="chart-card" style={{ minWidth: 220 }}>
      <div className="chart-title" style={{ marginBottom: 14 }}>Alerts Overview</div>
      {[
      ...
      <div style={{ marginTop:14 }}>
        <Link to="/analyst" className="btn" style={{ width:"100%", justifyContent:"center" }}>
          + New Screening
        </Link>
      </div>
    </div>
...
export default function ScreeningsList() {
  ...
  const filtered = useMemo(()=>{
    const n=norm(searchName); const r=filterRisk.toUpperCase();
  ...
  function toggleAll(){
    setSelectedIds(selectedIds.size===pageItems.length&&pageItems.length>0?new Set():new Set(pageItems.map(r=>r.id)));
  }
    ...
    <>
      ...
      <div className="page-header" style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
        <div>
          ...
          <div className="page-subtitle">{busy?"Chargement…":`${total} screening${total!==1?"s":""} au total`}</div>
        </div>
      ...
      <div className="stat-pills">
        <div className={`stat-pill all`} style={{cursor:"pointer"}} onClick={()=>{setFilterRisk("");setPage(1);}}>
        ...
        <div className="stat-pill medium" style={{cursor:"pointer"}} onClick={()=>{setFilterRisk("MEDIUM");setPage(1);}}>
          <span className="pill-count">{busy?"…":stats.medium}</span>
          <span className="pill-label">Medium Risk</span>
        </div>
        <div className="stat-pill low" style={{cursor:"pointer"}} onClick={()=>{setFilterRisk("LOW");setPage(1);}}>
          ...
          <div className="filters-bar" style={{marginBottom:14}}>
            <div className="row" style={{justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
              <div className="filter-chips">
                ...
                <span className="filter-label" style={{marginLeft:8}}>|</span>
                <span className="filter-label">Status:</span>
                {["","PENDING","DONE","RUNNING"].map(s=>(
                  <button key={s||"all"}
                    onClick={()=>{setFilterStatus(s);setTimeout(load,0);setPage(1);}}
                    style={{
                      padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:600,cursor:"pointer",
              ...
              <div className="row" style={{gap:8}}>
                <input className="input" style={{width:180,padding:"5px 10px",fontSize:12}}
                  placeholder="Rechercher un nom…" value={searchName}
                  onChange={e=>{setSearchName(e.target.value);setPage(1);}} />
                <button className="btn secondary sm" onClick={()=>{setFilterRisk("");setFilterStatus("");setSearchName("");load();}}>
                  Clear Filters
                </button>
          ...
          <div className="screen" style={{padding:0,overflow:"hidden"}}>
            <div style={{padding:"12px 16px 10px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid var(--border)"}}>
              <div className="row" style={{gap:10}}>
                {selectedIds.size>0 && (
                  <>
                    <button className="btn secondary sm">📋 Batch Review</button>
                    <button className="btn secondary sm">Assign Cases</button>
                  </>
                )}
            ...
            <div style={{overflowX:"auto"}}>
              <table className="cases-table" style={{width:"100%"}}>
                <thead>
                  <tr>
                    <th style={{width:36}}><input type="checkbox" checked={selectedIds.size===pageItems.length&&pageItems.length>0} onChange={toggleAll}/></th>
                    <th>Name ↑</th>
                    <th>Category</th>
                    <th>Risk Level ↑</th>
                    <th>Status ↑</th>
                ...
                <tbody>
                  {busy&&all.length===0?(
                    <tr><td colSpan={6} style={{textAlign:"center",padding:"40px 0",opacity:0.4}} className="small">Chargement…</td></tr>
                  ):pageItems.length===0?(
                    <tr><td colSpan={6}>
                      <div className="empty-state">
                        <div className="empty-state-icon">🔍</div>
                        <div className="empty-state-title">Aucun résultat</div>
                        <div className="empty-state-sub">Modifiez vos filtres ou lancez un nouveau screening.</div>
                      </div>
                    </td></tr>
                  ):pageItems.map(r=>{
                    ...
                    return (
                      <tr key={r.id} className={selectedIds.has(r.id)?"selected":""}>
                        ...
                        <td>
                          <div style={{display:"flex",alignItems:"center",gap:10}}>
                            ...
                            <div>
                              ...
                              <div className="small" style={{opacity:0.55,marginTop:1}}>{fmtDate(r.created_at)}</div>
                            </div>
                          </div>
                        </td>
                        <td className="small" style={{color:"var(--text-secondary)"}}>{cat}</td>
                        <td>
                          {risk==="HIGH"?   <span className="risk-badge high">High Risk</span>:
                           risk==="MEDIUM"? <span className="risk-badge medium">Medium Risk</span>:
                           risk==="LOW"?    <span className="risk-badge low">Low Risk</span>:
                        ...
                        <td>
                          <button className="btn secondary sm" onClick={()=>navigate(`/screenings/${encodeURIComponent(r.id)}`)}>
                            View Details
                          </button>
                        </td>
                      </tr>
            ...
            <div style={{padding:"12px 16px",borderTop:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <span className="small" style={{color:"var(--text-muted)"}}>{PAGE_SIZE} par page</span>
              <Pagination current={page} total={pageCount} onChange={setPage}/>
            </div>
          </div>
        </div>
